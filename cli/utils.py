...
def select_mandate(default=None) -> str:
    ...
    mandate needs no CLI edit and the list can never drift. ``default`` is the
    previous run's answer, offered back as the highlighted choice.
    """
    ...
    choice = questionary.select(
        ...
        # A remembered mandate that this build no longer registers is not among
        # the choices, and questionary rejects a default it cannot find.
        default=default if any(default == value for _, value in options) else None,
        instruction=(
