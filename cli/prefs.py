...
def sanitize(prefs: dict, asset_type) -> dict:
    ...
    # "" (no mandate) is a real answer and must round-trip, so this checks
    # membership rather than truthiness. A mandate retired between versions
    # drops out, like a retired model.
    from tradingagents.mandates import list_mandates

    if prefs.get("mandate") in {"", *(m.name for m in list_mandates())}:
        kept["mandate"] = prefs["mandate"]
