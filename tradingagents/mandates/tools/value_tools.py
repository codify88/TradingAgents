...
def _screens_block(screens) -> str:
    lines = ["### Screens (equity_value hard disqualifiers the statements can settle)"]
    for s in screens:
    ...
    lines.append(
        ...
        "NO DATA means the statements cannot settle it; say so, do not guess."
    )
    return "\n".join(lines)
...
def _unavailable(tool_name: str, ticker: str, err: Exception) -> str:
    return (
...
def _guarded(fn):
    ...
    def wrapper(ticker: str, curr_date: str) -> str:
        try:
            ...
            return _unavailable(fn.__name__, ticker, e)
    return wrapper
