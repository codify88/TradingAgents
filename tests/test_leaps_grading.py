...
def _chain_on(entry_quotes, exit_quotes):
    ...
    def chain(symbol, date):
        ...
        return tuple(
            op.Contract(f"C{k:g}", EXPIRY, k, "call", bid, ask, 1000, 10)
            for k, (bid, ask) in quotes.items()
        )
    return chain


def _grade(frame, entry_quotes, exit_quotes, target=0.75):
    with patch("tradingagents.mandates.tools.financials.alpha_vantage_daily_strict", return_value=frame), \
         patch.object(op, "option_chain", side_effect=_chain_on(entry_quotes, exit_quotes)), \
         patch.object(op, "risk_free_rate", return_value=0.04), \
         patch.object(op, "dividend_yield", return_value=0.0):
        return lg.grade("X", ENTRY.strftime("%Y-%m-%d"), 126, target)
...
class TestGrade:
    ...
    def test_edge_against_matched_shares(self):
        ...
        o = lg.OptionOutcome("C", 0.75, delta=0.8, entry_ask=10.0, exit_value=26.0,
    ...
    def test_forgone_dividends_count_against_the_call(self):
        o_plain = lg.OptionOutcome("C", 0.75, 0.8, 10.0, 26.0, DAYS[136], 100.0, 0.20)
        o_div = lg.OptionOutcome("C", 0.75, 0.8, 10.0, 26.0, DAYS[136], 100.0, 0.23)
...
class TestReview:
    ...
    def test_render_compares_chosen_calls_with_calls_passed_on(self):
        good = lg.OptionOutcome("C", 0.75, 0.8, 10.0, 30.0, DAYS[136], 100.0, 0.20)
        bad = lg.OptionOutcome("C", 0.75, 0.8, 10.0, 12.0, DAYS[136], 100.0, 0.20)
    ...
    def test_nothing_to_grade_says_why(self):
        assert "only a position" in lg.render([])
